 ...
 */
public enum StatusExplodeForTransformToNull {
	...
	private static class Lenient {
		private static final Map<String, StatusExplodeForTransformToNull> names = new HashMap<>();
	}

	private StatusExplodeForTransformToNull(String... lenientNames) {
		safeMap(this).from(lenientNames).includeSelf(e -> lenientNames.length == 0 ? null : e.name()).withStore(Lenient.names);
	}

	public static Optional<StatusExplodeForTransformToNull> lenientValueOf(String lenientName) {
